...
import { updateKhuyenMaiAction, getAllLoaiPhongAction, getKhuyenMaiByIdAction } from "./actions";
...
export default function EditKhuyenMai({ maKhuyenMai, onClose }: Props) {
    ...
    useEffect(() => {
        async function fetchData() {
            setIsLoading(true);
            const [lpRes, kmRes] = await Promise.all([
                getAllLoaiPhongAction(),
                getKhuyenMaiByIdAction(maKhuyenMai)
            ]);
            ...
            if (kmRes.success && kmRes.data) {
                const km = kmRes.data;
                setMaCode(km.maCode);
                setTenKhuyenMai(km.tenKhuyenMai);
                setNoiDung(km.noiDung);
                setNgayBatDau(km.ngayBatDau);
                setNgayKetThuc(km.ngayKetThuc);
                setSelectedDetails(km.chiTiet.map((ct: any) => ({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        startTransition(async () => {
            const result = await updateKhuyenMaiAction(maKhuyenMai, {
                maCode,
                tenKhuyenMai,
                noiDung,
                ngayBatDau,
    ...
    if (isLoading) {
        return (
            <div className="fixed inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm z-[9999]">
                <div className="bg-white p-8 rounded-2xl">Đang tải dữ liệu...</div>
            </div>
        );
    }
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm z-[9999]">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-8 mx-4">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-blue-600">Chỉnh sửa khuyến mãi</h2>
